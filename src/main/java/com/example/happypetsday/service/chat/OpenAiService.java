...
//  @Value("${gpt.api}") String apiKey = chatGpt api 키 https://openai.com/blog/introducing-chatgpt-and-whisper-apis
...
//     밑의 코드에서 system부분은 한글로 하면 원하는 결과가 나오지 않을 때도 있어서 영어로 했음
...
//        이 부분은 없어도 되기는 하지만 이전의 채팅내용을 기반으로 다음 질문에 대한 답변을 생성하기 때문에
//        우리서비스에 대해 물어보는 질문을 넣어주고 내가 원하는 가장 이상적인 답변을 설정해주었다.
